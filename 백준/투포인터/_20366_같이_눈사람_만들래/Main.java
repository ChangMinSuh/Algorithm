...
- 시간초과 날 것이라 생각하고 돌렸는데 맞아서 당황스럽다.
---
- list에는 눈사람을 만들 수 있는 모든 경우의 값을 삽입한다. (최대 600x599 = 359,400, 거의 360,000)
- list는  ListData라는 클래스를 타입으로 사용한다
    - ListData
        - data: 두 지름을 더한 값
        - index1: 한 지름의 인덱스
        - index2: 또 다른 지름의 인덱스
- list를 정렬한다. (대략 360,000 * 600 = 216,000,000)
- for문을 돌려 인접한 눈사람을 비교한다. (O(N), 대략 360,000)
